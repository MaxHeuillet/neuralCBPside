...
class NeuralUCBDiag:
    ...
    def train(self, context, reward): #update the agent
        self.context_list.append(torch.from_numpy(context.reshape(1, -1)).float()) #historique des contextes
        self.reward.append(reward) #historique des rewards
        # print(self.reward)
        ...
        while True:
            ...
            for idx in index:
                ...
                print(r.shape)
                optimizer.zero_grad()
                delta = self.func(c.cuda()) - r #difference entre le gain predit et le gain qui a ete recu
                loss = delta * delta
                loss.backward()
                optimizer.step()
