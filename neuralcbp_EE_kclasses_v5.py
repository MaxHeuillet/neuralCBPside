...
def EE_forward(game, net1, net2, x):
    ...
    for k in range(game.N):
        ...
        f1[k].sum().backward(retain_graph=True)
    ...
    return f1, f2, dc_normalized.unsqueeze(0)
...
class CBPside():
    ...
    def convert_pred_format(self,pred):
        for k in range(self.game.N):
            pred[k] = pred[k][0].detach().cpu().numpy()
        return pred
    
    def convert_conf_format(self,pred):
        for k in range(self.game.N):
            pred[k] = pred[k].detach().cpu().numpy()
            pred[k] = np.array(max(pred[k]))
        return pred
