...
def getParetoOptimalActions(LossMatrix, N, M, halfspace):
    ...
    for z in range(N):
        ...
        m = lp.LpProblem(name="Pareto_Optimization", sense=lp.LpMinimize)
        ...
        for i2 in range(N):
            if i2 != z:
                lossExpr = lp.lpSum((LossMatrix[i2][j] - LossMatrix[z][j]) * vars[j] for j in range(M))
                lossConstStr = 'c_{}'.format(i2)
        ...
        for element in halfspace:
            ...
            if sign != 0:
                halfspaceExpr = lp.lpSum(
                    sign * (LossMatrix[pair[0]][j] - LossMatrix[pair[1]][j]) * vars[j]
                    for j in range(M)
                )
