...
class CBPside():
    ...
    def obtain_probability(self,  t , factor):
        ...
        self.prob_distribution = self.sampler.get_prob_distribution()
        ...
        idx = np.random.choice(range(self.K), p= self.prob_distribution)
        self.idx = idx
    ...
    def get_action(self, t, X):
        ...
        if t < self.N:
            ...
            for pair in self.mathcal_N:
                ...
                halfspace_exploit.append( ( pair, np.sign(tdelta) ) )
            
            # print('halfspace', halfspace)
            # print('halfspace_exploit', halfspace_exploit)

            P_t = self.pareto_halfspace_memory(halfspace)
            N_t = self.neighborhood_halfspace_memory(halfspace)

            P_t_exploit = self.pareto_halfspace_memory(halfspace_exploit)
            N_t_exploit = self.neighborhood_halfspace_memory(halfspace_exploit)

            # print('Pareto', P_t, P_t_exploit )
            # print('Neigbors', N_t, N_t_exploit )
            ...
            Nplus_t_exploit = []
            for pair in N_t_exploit:
                Nplus_t_exploit.extend( self.N_plus[ pair[0] ][ pair[1] ] )
            Nplus_t_exploit = np.unique(Nplus_t_exploit)
            ...
            exploit1= np.union1d(  P_t_exploit, Nplus_t_exploit )
            self.exploit1 = np.array(exploit1, dtype=int).item()
            print('exploit', self.exploit1)
    ...
    def update(self, action, feedback, outcome, t, X):
        ...
        if action == 0 and t>self.N:
            ...
            if (self.exploit1 == 1 and outcome == 0) or (self.exploit1 == 2 and outcome ==1):
                reward = np.array( [ 1 if i<=self.idx else 0 for i in range(self.K) ] )
                x_tilde = reward / ( self.prob_distribution + gamma)
                self.sampler.weights = self.sampler.weights * np.exp( eta * x_tilde )

            else:
                reward = np.array( [ 0 if i<=self.idx else 1 for i in range(self.K) ] )
                x_tilde = reward / ( self.prob_distribution + gamma)
                self.sampler.weights = self.sampler.weights * np.exp( eta * x_tilde )
